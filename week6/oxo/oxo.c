...
validity valid(game *g, char *text) {
    ...
    if ((text[0] >= 100) || (isdigit(text[0]) == true)) return BadLetter;

    if (((text[1] - '0') == 0) || ((text[1] - '0') > 3))  return BadDigit;
...
player won(game *g) {
    ...
    //if (line((g->grid[0][0]), (g->grid[0][1]),(g->grid[0][2])) == g->next) return g->next;

    //Check all rows.
    //if (line(g->grid,0,X) || line(g->grid,0,X) || line(g,0,X)) return X;
    if ((g->grid[0][0]==X) && (g->grid[0][1]==X) && (g->grid[0][2]==X)) return X;
    if ((g->grid[1][0]==X) && (g->grid[1][1]==X) && (g->grid[1][2]==X)) return X;
    if ((g->grid[2][0]==X) && (g->grid[2][1]==X) && (g->grid[2][2]==X)) return X;

    if ((g->grid[0][0]==O) && (g->grid[0][1]==O) && (g->grid[0][2]==O)) return O;
    if ((g->grid[1][0]==O) && (g->grid[1][1]==O) && (g->grid[1][2]==O)) return O;
    if ((g->grid[2][0]==O) && (g->grid[2][1]==O) && (g->grid[2][2]==O)) return O;

    //Check all columns.
    if ((g->grid[0][0]==X) && (g->grid[1][0]==X) && (g->grid[2][0]==X)) return X;
    if ((g->grid[0][1]==X) && (g->grid[1][1]==X) && (g->grid[2][1]==X)) return X;
    if ((g->grid[0][2]==X) && (g->grid[1][2]==X) && (g->grid[2][2]==X)) return X;

    if ((g->grid[0][0]==O) && (g->grid[1][0]==O) && (g->grid[2][0]==O)) return O;
    if ((g->grid[0][1]==O) && (g->grid[1][1]==O) && (g->grid[2][1]==O)) return O;
    if ((g->grid[0][2]==O) && (g->grid[1][2]==O) && (g->grid[2][2]==O)) return O;

    //Check diagonals.
    if ((g->grid[0][0]==X) && (g->grid[1][1]==X) && (g->grid[2][2]==X)) return X;
    if ((g->grid[0][2]==X) && (g->grid[1][1]==X) && (g->grid[2][0]==X)) return X;

    if ((g->grid[0][0]==O) && (g->grid[1][1]==O) && (g->grid[2][2]==O)) return O;
    if ((g->grid[0][2]==O) && (g->grid[1][1]==O) && (g->grid[2][0]==O)) return O;
...
void printInvalid(validity v) {
    ...
    if (v == BadLetter) printf("Type a letter a, b or c");
    if (v == BadDigit) printf("Type a digit 1, 2 or 3");
    if (v == BadCell) printf("Choose an empty cell");
    printf("\n");
}
...
void play(char player) {
    ...
    while (! drawn(g)) {
        ...
        if (won(g)==X) {
            printf("\v---X WINS!---\v\n");
            printf("Final board displayed below.\v\n");
            break;
        }
        if (won(g)==O) {
            printf("\v---O WINS!---\v\n");
...
void testWin(game *g) {
    *g = (game) {{{X,X,X},{N,O,N},{N,O,N}},O,5}; //1
    assert(won(g) == X);
    *g = (game) {{{N,O,N},{X,X,X},{N,O,N}},O,5}; //2
    assert(won(g) == X);
    *g = (game) {{{N,O,N},{N,O,N},{X,X,X}},O,5}; //3
    assert(won(g) == X);
    *g = (game) {{{O,N,N},{O,X,N},{O,N,X}},X,5}; //4
    assert(won(g) == O);
    *g = (game) {{{N,O,N},{X,O,N},{N,O,X}},X,5}; //5
    assert(won(g) == O);
    *g = (game) {{{N,N,O},{X,N,O},{N,N,O}},X,5}; //6
    assert(won(g) == O);
    *g = (game) {{{X,N,O},{N,X,O},{N,N,X}},O,5}; //7
    assert(won(g) == X);
    *g = (game) {{{X,N,O},{N,O,X},{O,N,N}},X,5}; //8
    assert(won(g) == O);
}
...
int main(int n, char *args[n]) {
    if (n == 1) test();
        ...
        printf("\v---Welcome to OXO by Nikolay!---\n\v");
        play(X);
    }
    else if (n == 2 && strcmp(args[1],"O") == 0) {
        printf("---Welcome to OXO by Nikolay!---\n");
        play(O);
    }
    else {
