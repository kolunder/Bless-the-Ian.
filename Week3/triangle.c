...
// Validity check. 
//Convert string into integer and back and compare result with initial string.
bool isValid (const char *n) {
    char str[12];
    int number = convert(n);
    sprintf(str, "%d", number);

    if ((strcmp(n, str) == 0) && (number > 0)) {
        return true;
    } else {
        return false;
    }
...
long triangle(const char sa[], const char sb[], const char sc[]) {
    
    long a = convert(sa);
    long b = convert(sb);
    long c = convert(sc);

    if ((isValid(sa) == false) || (isValid(sb) == false) || (isValid(sc) == false)) {
        return Illegal;
    } else {
        if ((a == b) && (b == c)) {
            return Equilateral;
